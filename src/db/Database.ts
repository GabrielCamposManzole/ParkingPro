...
export default class Database
  implements IRepositorioVagas, IRepositorioVeiculos, IRepositorioClientes
{
  ...
  private getFactory(tipo: TipoVeiculo): VagaFactory | null {
    switch (tipo) {
      case TipoVeiculo.CARRO:
        return new VagaCarroFactory();
      case TipoVeiculo.MOTO:
        return new VagaMotoFactory();
      case TipoVeiculo.CAMINHAO:
        return new VagaCaminhaoFactory();
      default:
        return null;
    }
  }
  ...
  public addVaga(tipo: TipoVeiculo, numero: number): boolean {
    const vagaExistente = this.listarVagas().find(
      (v) => v.getNumero() === numero
    );
    if (vagaExistente) {
      return false;
    }
  ...
  public buscarVagaPorPlaca(placa: string): Vaga | undefined {
    return this.listarVagas().find(
      (vaga) => vaga.getVeiculoEstacionado()?.getPlaca() === placa
    );
  }
  ...
  public removerVeiculoPorPlaca(placa: string): boolean {
    const index = this.veiculosEstacionados.findIndex(
      (v) => v.getPlaca() === placa
    );
    if (index !== -1) {
  ...
  public buscarVeiculosPorCpfCliente(cpf: string): Veiculo[] {
    return this.listarTodosCadastrados().filter(
      (veiculo) => veiculo.getCliente()?.getCpf() === cpf
    );
  }
  ...
  public atualizar(
    cpf: string,
    novosDados: { nome?: string; tipo?: ClientType }
  ): Cliente | null {
  ...
  public excluir(cpf: string): boolean {
    const index = this.clienteDB.findIndex(
